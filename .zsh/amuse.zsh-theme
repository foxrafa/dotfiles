...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
  ...
  prompt='
╭─ %{$fg_bold[cyan]%}%~%{$reset_color%} %{$fg_bold[magenta]%}%{$(parse_git_branch)%}%{$reset_color%} %{$fg_bold[yellow]%}❯%{$reset_color%} %{$fg_bold[cyan]%}${(U)USER}%{$reset_color%} %{$fg_bold[red]%}%*%{$reset_color%}
╰─❯ '
else
  # Mac: check username
  if [[ "${USER}" == "fox" ]]; then
